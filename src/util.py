...
def safe_name(id: Union[str, int], unsafe_name: str) -> str:
    ...
    # TODO: Replace this with something nicer like urlencode?
    replaced_name = unsafe_name.lower().strip().replace(' ', '_').replace('/', '-').replace(':', '').replace('?', '').replace('|','').replace('"', '').replace("'", '').replace('<', '').replace('>', '')
